...
public class GenericDataLoader : DataLoader
{
    public async override Task LoadDataAsync(Stream fileStream, ApplicationDbContext db)
    {
        ...
        foreach (GenericSession item in
            JsonSerializer.Deserialize<List<GenericSession>>(text) ?? new())
        {
    ...
    public override Task LoadSessionizeDataAsync(string url, ApplicationDbContext db)
    {
